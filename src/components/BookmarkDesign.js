...
const BookmarkDesign = () => {
  ...
  const { handleOnBookmark, BookMarkDate } = context;
  const CopyLink = (data) => {
    let Text = data;
              ...
              <div className="col-lg-4 p-5" key={ele.webformatURL}>
                <div className="card" style={{ width: "30rem" }}>
                  ...
                  <div className="card-body d-flex flex-row flex-wrap justify-content-between">
                    ...
                    <button type="button" className="btn btn-outline-primary" onClick={() => { CopyLink(ele.webformatURL) }}><i class="bi bi-link-45deg"></i></button>
                  </div>
                </div>
              </div>
            )
